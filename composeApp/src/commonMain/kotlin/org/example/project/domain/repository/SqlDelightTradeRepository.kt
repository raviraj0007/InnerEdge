...
class SqlDelightTradeRepository(
    ...
    override fun getAllTrades(): Flow<List<Trade>> {
        return queries.selectAllTrades()
            ...
            .map { entities -> entities.map(::mapEntityToTrade) }
    }

    override suspend fun getTradeById(id: String): Trade? {
        return queries.selectTradeById(id).executeAsOneOrNull()?.let(::mapEntityToTrade)
    }
    ...
    override suspend fun deleteTrade(id: String) {
        queries.deleteTradeById(id)
    }

    private fun mapEntityToTrade(entity: TradeEntity): Trade {
        return Trade(
            id = entity.id,
